...
import type { ExistingNgo } from './NgoForm'

type TambahNgoModalProps = {
  open: boolean
  onClose: () => void
  onSuccess: () => void
  negeriTetap?: string
...
export function TambahNgoModal({ open, onClose, onSuccess, negeriTetap, existing }: TambahNgoModalProps) {
  return (
    <AnimatePresence>
      {open && (
        <motion.div
        ...
        >
          ...
          >
            ...
            <h2 className="mb-6 text-base font-semibold text-neutral-900">
              {existing ? 'Kemas Kini NGO' : 'Tambah NGO'}
            </h2>

            <NgoForm onSuccess={onSuccess} negeriTetap={negeriTetap} existing={existing} />
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
