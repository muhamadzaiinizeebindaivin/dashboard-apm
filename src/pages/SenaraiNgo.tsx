...
export function SenaraiNgo() {
  ...
  const muatSemula = useCallback(async () => {
    setLoading(true)
    const { data } = await supabase.from('ngo').select('negeri')
    ...
    <div>
      <div className="mt-2 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/60 bg-white/40 px-5 py-4 shadow-lg backdrop-blur-md">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-neutral-900">Senarai NGO</h1>
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        {NEGERI_LIST.map((negeri, i) => (
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={() => navigate(`/ngo/${encodeURIComponent(negeri)}`)}
